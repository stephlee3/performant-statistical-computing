...
`determine_precision` - figure out the lower bound and upper bound of the smallest machine distinguishable values, which could also be obtained from `.Machine$double.neg.eps` and `.Machine$double.eps`.
...
# Fill in
count_duplicate <- function(n_draw, seed = 42){
  set.seed(seed)
  U <- runif(n_draw) 
  I <- duplicated(U)
  C <- sum(I)
...
obs_dup = count_duplicate(n_draw = n_draw)
exp_dup = est_num_expected_duplicate(n_draw = n_draw, n_choice = n_unique_double)
print(obs_dup)
print(exp_dup)

exp_num_coll <- function(n, k) n + 2^k * expm1(n * log1p(-2^(-k)))
print(exp_num_coll(n = 1e6, k = 32))
```
The empirical number of duplicates is 123, whereas the theoretical expected value for $2^{53}$ unique machine-representable values in $[0, 1]$ is around 0.0001. But if we plug in $k=32$ in the formula provided by the paper, we can obtain `exp_num_coll`$\approx 116$, which is close to our empirical results. Therefore, one possible explanation is that the machine is under 32-bit setup. 
...
The relative error first decreases then increases as the stepsize becomes smaller. 
...
Let $g(\Delta x) = \frac{f(x + \Delta x) - f(x - \Delta x)}{2 \Delta x}- f'(x)$. We try to find $\Delta x$ s.t. $g(\Delta x) \approx  0$. We can apply Newton's method to iteratively find the root.
...
algo_conv_test = function(optim_result){
  if (optim_result$convergence!=0){
    warning("the optim algorithm failed to converge.")
  }
}
algo_conv_test(optim_result)
```
\emph{Why it didn't work?} `exp(logit_prob)` will reach infinity when regression coefficient is vector of 1. 
...
approximate_grad <- function(func, x, dx = 10^-6) {
  ...
  for (i in 1:length(x)) {
    # Fill in
    dxi = rep(0,length(x)); dxi[i] = dx
    numerical_grad[i] = (func(x+dxi)-func(x-dxi))/(2*dx)
  }
...
compute_logp <- function(x) {
  # Fill in
  sum(x*log(0.3)+ (1-x)*log(0.7))
}
compute_grad <- function(x) {
  # Fill in
  rep(log(0.3)-log(0.7),length(x))
}
...
compute_logit_grad <- function(
  ...
  # Fill in
  loglik_grad <- t(design_mat) %*% n_success - t(t(n_trial) %*% (design_mat * as.vector((exp(design_mat %*% reg_coef))/(1+exp(design_mat %*% reg_coef)))))   
...
print(optim_result$convergence) # successful!
```
...
profvis({
 ...
 approx_grad <- approximate_grad(compute_internet_ad_ploglik, reg_coef, dx)
})
```
...
approx_directional_deriv <- function(func,x,v,t = 1e-6) {
  # Fill in
  (func(x+t*v)-func(x-t*v))/(2*t)
...
assert(assertion, {
  ...
  # Fill in
  grad <- compute_internet_ad_grad(reg_coef)
  grad_v_prod <- sum(grad*v)
  are_all_close(grad_v_prod, approx_dderiv)
})
```

For Hessian matrix, we can use the following formula to test our calculation:
$$\begin{align*}
H(x) v = \frac{1}{t} (\nabla f(x+tv) - \nabla f(x))+ O(t)
\end{align*}$$