...
```{r setup, echo=FALSE}
required_packages <- c('Rcpp', 'RSpectra', 'testit', 'bench')
...
v = rep(1, ar_length)
yr = ar_precision_matvec_R(v, auto_corr)
yc = ar_precision_matvec(v, auto_corr)

assert("Rcpp function gives the same result as R function", {
  are_all_close(yr, yc)
})
```
...
bench::mark(ar_precision_matvec_R(v, auto_corr))
bench::mark(ar_precision_matvec(v, auto_corr))
```
<font color="blue">
For the matrix-vector multiplication, Rcpp is faster than R implementation. 
</font>
...
```
<font color="blue">
For the PCA, Rcpp + Lanczos algorithm is way faster than direct egien decomposition. Note: we set `eval = FALSE` in  the above code chunk since eigen decomp could be time consuming. 
</font>
...
<font color="blue">
The `sym_banded_matvec` has similar (slightly better) performance compared with `ar_precision_matvec`. The vector size could affect the relative performance.  
</font>

```{r}
banded_mat = cbind(c(1, rep(1 + auto_corr^2, ar_length - 2), 1), c(rep(- auto_corr, ar_length - 1), 0))
banded_mat = banded_mat / (1 - auto_corr^2)
yb = sym_banded_matvec(v, banded_mat)

assert("Rcpp function using symmetric banded matrices gives the same result as R function", {
  are_all_close(yr, yb)
})

bench::mark(sym_banded_matvec(v, banded_mat))
bench::mark(ar_precision_matvec(v, auto_corr))
...
AB.R = A %*% B
AB.C = row_col_dot_matmat(A, B)
assert("Rcpp function gives the same result as R function", {
  are_all_close(AB.R, AB.C)
})
...
<font color="blue">
The `col_oriented_matmat` is faster. The `row_col_dot_matmat` is a naive algorithm with triple for-loops that takes $O(nmr)$ time, while `col_oriented_matmat` decomposes the matrix-matrix multiplication into $m$ matrix-vector multiplication which could avoid some for-loops.    
</font>

```{r}
# Check and benchmark
AB.col = col_oriented_matmat(A, B)
assert("Rcpp function col_oriendted_matmat gives the same result as R function", {
  are_all_close(AB.R, AB.col)
})
...
<font color="blue">
Performance: `col_oriented_matmat` > built-in R function > `row_col_dot_matmat`.
</font>
...
```{r}
# Check and benchmark
AB.eigen = matmat_via_eigen(A, B)
assert("Rcpp function matmat_via_eigen gives the same result as R function", {
  are_all_close(AB.R, AB.eigen)
})
bench::mark(matmat_via_eigen(A, B))
```

<font color="blue">
`matmat_via_eigen` apparently outperforms other methods. 
</font>